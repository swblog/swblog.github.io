 ...
 return $.extend({
   ...
   template: '<%var arr=location.host.split("."); var isLocalhost = arr.length===1; var username= isLocalhost ? "swblog" : arr[0]; %>'+
     ...
     '  <li class="previous"><a data-on="?m=back">← 返回</a></li>' +
      ...
      '<%=username%>/<%=username%>.github.io/issues?utf8=%E2%9C%93&q=<%=decodeURIComponent(isLocalhost ? "localhost测试评论" : obj.time)%>">查看评论</a></li>' +
      ' <li class="next"><a target="_blank" href="https://github.com/'+
      '<%=username%>/<%=username%>.github.io/issues/new?title=<%=decodeURIComponent(isLocalhost ? "localhost测试评论" : obj.time)%>">去评论 &rarr;</a></li>' +
     '</ul>'
 }, option);
};
